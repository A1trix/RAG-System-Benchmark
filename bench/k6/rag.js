...
const endpoint = __ENV.RAG_ENDPOINT_URL || "http://rag-pipeline:8080/query";
const apiToken = __ENV.RAG_API_TOKEN || __ENV.API_TOKEN || "";
...
const failFastOnTotalFailure = (__ENV.K6_FAIL_FAST_ON_TOTAL_FAILURE || "1") === "1";

const promptMetricThresholds = buildPromptMetricThresholds(promptIds);

const thresholds = {};
for (const [key, value] of Object.entries(promptMetricThresholds)) {
	thresholds[key] = value;
}
if (failFastOnTotalFailure) {
	thresholds.http_req_failed = [
		{
			threshold: "rate<1",
			abortOnFail: true,
			delayAbortEval: __ENV.K6_FAIL_FAST_DELAY || "20s",
		},
	];
}

const arrivalRate = __ENV.K6_ARR_RATE ? parseInt(__ENV.K6_ARR_RATE, 10) : null;
const useArrivalRate =
	arrivalRate !== null && !Number.isNaN(arrivalRate) && arrivalRate > 0;
if (!useArrivalRate) {
...
export default function () {
	...
	const payload = JSON.stringify({
		...
		sessionId: useArrivalRate
			? `k6-rag-${runId}-${scenarioName}-${iterationInTest}`
			: `k6-rag-${runId}-${__VU}-${__ITER}`,
		prompt_id: promptId,
		request_meta: {
			vu: __VU,
			iter: __ITER,
	...
	answer_chars.add(answer ? answer.length : 0);
	const hasCitation = (answer || "").includes("[source:");
	const success =
		res.status === 200 &&
		contractOk &&
	...
	check(res, {
		...
		"contract: json answer": () => contractOk,
		"answer non-empty": () => (answer || "").trim().length > 0,
	...
	if (inMeasureWindow) {
		...
		// Extract token usage from response for correlation analysis
			const tokenPrompt = obj && typeof obj.prompt_tokens === "number" ? obj.prompt_tokens : null;
			const tokenCompletion = obj && typeof obj.completion_tokens === "number" ? obj.completion_tokens : null;
			const tokenTotal = obj && typeof obj.total_tokens === "number" ? obj.total_tokens : null;
			if (tokenPrompt !== null) tokens_prompt.add(tokenPrompt);
			if (tokenCompletion !== null) tokens_completion.add(tokenCompletion);
			if (tokenTotal !== null) tokens_total.add(tokenTotal);
		} else {
